...
const productListPriceHistoryController = require('../Controllers/productListPriceHistoryController');
    ...
    if (body !== null && body !== undefined) {
        if (body.hasOwnProperty('model') && body.hasOwnProperty('lotes')) {
            if (body.model.hasOwnProperty(inventario.frm_preC) && body.model.hasOwnProperty(inventario.frm_preP) &&
                ...
                var result = null;
                var c = 0;
                result = await productController.updateProdctForBill(body.model);
                if (result !== null) {

                    result = await productListPriceHistoryController.update(body.model);
                    if (result !== null) {
                        console.log("Respuesta update Price History-->", result);
